...
```{r}
coffee_type_models <- weekly_coffee_volume %>%
...
autoplot(forecast_coffee_type, weekly_coffee_volume) +
   labs(title = "8-Week Coffee Type Forecasts (Seasonal ARIMA)",
       y = "Amount", x = "Week") +
  facet_wrap(~coffee_name, scales = "free_y") +
  theme_minimal()
```
...
autoplot(forecast_revenue, weekly_revenue) +
   labs(title = "8-Week Revenue Forecasts (Seasonal ARIMA)",
       y = "USD", x = "Week") +
  theme_minimal()
```
