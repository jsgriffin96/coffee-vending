...
acf_data %>%
  autoplot() +
  facet_wrap(~ coffee_name, scales = "free_y") +
  labs(title = "ACF of Weekly Coffee Volume by Coffee Type",
       x = "Lag", y = "ACF") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none")
...
# tsibble
weekly_volume_diff_2_tsibble <- weekly_volume_diff_2 %>%
  filter(!is.na(diff_volume_2)) %>%
  as_tsibble(index = week, key = coffee_name)

#  ACF
acf_diff2_data <- weekly_volume_diff_2_tsibble %>%
  ACF(diff_volume_2)
...
acf_diff2_data %>%
  autoplot() +
  facet_wrap(~ coffee_name, scales = "free_y") +
  labs(title = "ACF After Second Differencing (Lag = 2)",
       x = "Lag", y = "ACF") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none")
