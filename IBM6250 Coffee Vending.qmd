...
weekly_long %>%
  ...
  group_by(metric) %>%
  group_walk(~{
    acf(.x$value, main = paste("ACF for", .y))  
  })
```
...
weekly_diff %>%
  group_by(metric) %>%
  group_walk(~{
    acf(.x$value_diff, lag.max = 30,
        main = paste("ACF for Differenced", unique(.y$metric)))
  })
```
...
#### Machine 2
```{r m2interactive-weekly-chart}
#| fig-cap: "Machine 2 Weekly ingredient demand vs. cups sold"
#| echo: false

weekly_sales_machine2 <- sales_ingredients |>
  filter(machine_id=="machine2") |>
  mutate(week = lubridate::floor_date(date, unit = "week")) |>
  group_by(week) |>
  summarise(across(coffee:vanilla, sum, na.rm = TRUE),
            sales_n = n()) |>
...
weekly_sales_machine2 <- weekly_sales_machine2|>
  as_tsibble(index = week)

# 1. Reshape to long format: one row per week-metric pair -------------
weekly_long_machine2 <- weekly_sales_machine2%>%filter() |>
  pivot_longer(
    cols      = coffee:sales_n,   # everything after the 'week' column
    names_to  = "metric",
    values_to = "value"
